...
| `heroLede` (optional) | string | Längerer Stimmungs-Absatz direkt unter der Tagline im Hero — nur bei einem landestypischen Charakterzug, der mehr als eine Zeile verdient (Beispiel: Marokkos doppelter Charakter zwischen mediterranem Einfluss und Sahara) |
...
| `neighborsHistoryNotes[]` (optional) | `{label, text, large?}` — `text` als String (ein Absatz) oder String-Array (mehrere Absätze) | Generische 2-Spalten-Zusatzinfo-Kacheln als EIGENE Kacheln neben Nachbarländer + Timeline, in Array-Reihenfolge gerendert; `large: true` macht eine Kachel bewusst höher/prominenter (Beispiel: Äthiopien nutzt das für die Koexistenz von Christentum und Islam, Marokko für seine geopolitische Doppelrolle mit mehreren Absätzen) |
...
| `dishesTeaser` (optional) | string | Überschreibt den generischen Gerichte-Section-Teaser aus `CountryPage.jsx` — nur bei einem landestypischen, ehrlichen Hinweis nutzen (Beispiel: Äthiopien weist auf die vegane Fastentradition der äthiopisch-orthodoxen Kirche hin) |
| `restaurantsTeaser` (optional) | string | Überschreibt den generischen Restaurant-Section-Teaser aus `CountryPage.jsx` — gleiches Prinzip wie `dishesTeaser` (Beispiel: Äthiopien weist auf die Überschneidung mit der eritreischen Küche/Gastronomie hin) |
...
| `HistoryTile` (`country/`) | Scrollbare Zeitleiste (`history[]`), 3 Spalten breit, feste Höhe (163px, wie eine normale Fakten-Kachel z. B. Bevölkerung) — steht neben Nachbarländer (ebenfalls 3 Spalten, ebenfalls feste 163px Höhe mit `overflow-y: auto` als Sicherheitsnetz) |
| `NeighborsHistoryGrid` (`country/`) | Zweiter `.fact-grid`-Block (Nachbarländer inkl. optionalen `neighborsNotes[]`-Zusatzhinweisen + `HistoryTile` + je einer `NoteTile` pro Eintrag in `neighborsHistoryNotes[]`) |
| `NoteTile` (`country/`) | Generische, optionale Textkachel (`{label, text, large?}`, `span`-Prop für die Breite, Default 2) für kulturellen/historischen/praktischen Zusatzkontext, der in kein anderes Fakten-Feld passt. `text` als String oder String-Array (mehrere `<p>`-Absätze) möglich. `large` macht sie höher/prominenter (per `min-height`, keine Grid-Zeilen-Überspannung, um keine Lücken im kleinen `NeighborsHistoryGrid` zu erzeugen). In `NeighborsHistoryGrid` mit `span={3}` verwendet, damit die Breite zu Nachbarländer/`HistoryTile` passt. Rendert `null` ohne `text` |

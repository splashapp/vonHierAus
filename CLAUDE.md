...
| `neighborsNote` (optional) | `{label, text}` | Kurzer Zusatzhinweis DIREKT IN der Nachbarländer-Kachel selbst, kleinere Schrift unter der Länderliste (Beispiel: Äthiopien nutzt das für einen Reisekombinations-Hinweis) |
| `neighborsHistoryNotes[]` (optional) | `{label, text, large?}` | Generische 2-Spalten-Zusatzinfo-Kacheln als EIGENE Kacheln neben Nachbarländer + Timeline, in Array-Reihenfolge gerendert; `large: true` macht eine Kachel bewusst höher/prominenter (Beispiel: Äthiopien nutzt das für die Koexistenz von Christentum und Islam) |
